...
WHERE c.defense_spending_score IS NOT NULL
   ...
   OR c.defense_composite_score IS NOT NULL
RETURN count(CASE WHEN c.defense_spending_score IS NOT NULL THEN 1 END)
       ...
       count(CASE WHEN c.arms_export_score IS NOT NULL THEN 1 END)
           AS has_arms_score,
       count(CASE WHEN c.defense_composite_score IS NOT NULL THEN 1 END)
...
run("defense_composite_score — must be 0-1, zero NULLs among populated", """
MATCH (c:Country)
WHERE c.defense_composite_score IS NOT NULL
RETURN count(c) AS total,
       round(max(c.defense_composite_score), 4) AS max_composite,
       round(min(c.defense_composite_score), 4) AS min_composite,
       round(avg(c.defense_composite_score), 4) AS avg_composite,
       count(CASE WHEN c.defense_composite_score > 1.0 THEN 1 END)
           AS above_one,
       count(CASE WHEN c.defense_composite_score < 0.0 THEN 1 END)
           AS below_zero
""")
...
run("Top 15 by defense_composite_score (US must be at or near top)", """
MATCH (c:Country)
WHERE c.defense_composite_score IS NOT NULL
RETURN c.name AS country,
...
ORDER BY c.defense_composite_score DESC LIMIT 15
""")
...
MATCH (c:Country)
WHERE c.defense_composite_score IS NOT NULL
RETURN count(CASE WHEN c.defense_composite_score > 0.5 THEN 1 END)
           AS critical,
       count(CASE WHEN c.defense_composite_score > 0.3
                   AND c.defense_composite_score <= 0.5 THEN 1 END)
           AS high,
       count(CASE WHEN c.defense_composite_score > 0.1
                   AND c.defense_composite_score <= 0.3 THEN 1 END)
           AS moderate,
       count(CASE WHEN c.defense_composite_score <= 0.1 THEN 1 END)
           AS low
""")
...
run("defense_composite_score vs live_risk_score — should broadly align", """
MATCH (c:Country)
WHERE c.defense_composite_score IS NOT NULL
...
RETURN c.name AS country,
       round(c.defense_composite_score, 4) AS composite_threat,
       round(c.live_risk_score, 3) AS live_risk,
       round(abs(c.defense_composite_score - c.live_risk_score), 4)
           AS delta
ORDER BY c.defense_composite_score DESC LIMIT 15
""")
